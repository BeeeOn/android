...
public class WidgetClockConfiguration extends WidgetConfiguration{
    ...
    private WidgetDevicePersistence mWidgetDevice2;
        ...
        if (!adapterId.isEmpty()) {
            for (int i = 0; i < mAdapters.size(); i++) {
                if (mAdapters.get(i).getId().equals(adapterId)) {
                    ...
                    foundIndex = Utils.getObjectIndexFromList(mWidgetDevice.getId(), mDevices);
                    if(foundIndex != -1) mSensorSpinner.setSelection(foundIndex);

                    mSensorSpinner2.setAdapter(dataAdapter);
                    foundIndex = Utils.getObjectIndexFromList(mWidgetDevice2.getId(), mDevices);
            ...
            doChangeAdapter(adapterId, mWidgetDevice.getId(), mWidgetDevice2.getId());
        }
    }
    ...
    public boolean saveSettings() {
        ...
        for(WidgetDevicePersistence dev : data.widgetDevices){
