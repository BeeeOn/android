...
public class DataOfLocationScreenActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		txtvwTitleLocation.setTextSize(getResources().getDimension(
				R.dimen.textsize));
		LinearLayout.LayoutParams titleLocationParams = new LinearLayout.LayoutParams(
		...
		if (devices != null) {
			...
			for (final BaseDevice device : devices) {
				...
				devicelayout.setLayoutParams(new RelativeLayout.LayoutParams(
				...
				txtvwLabelName.setTextSize(getResources().getDimension(
						R.dimen.textsizesmaller));
				RelativeLayout.LayoutParams txtvwLabelParams = new RelativeLayout.LayoutParams(
						LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
				...
				devicelayout.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						...
						intent.putExtra(Constants.DEVICE_CLICKED,
								device.getId());
				...
				RelativeLayout.LayoutParams rightObjectParams = new RelativeLayout.LayoutParams(
						LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
				...
				case Constants.TYPE_TEMPERATURE:
					...
					if (temperatureLayout != null
							&& temperatureLayout.getChildCount() > 0) {
						temperatureLayout.addView(getSplitter());
					}
					txtview.setText(device.getStringValue() + " "
							+ getString(device.getUnitStringResource()));
				...
				case Constants.TYPE_HUMIDITY:
					...
					if (humidityLayout != null
							&& humidityLayout.getChildCount() > 0) {
				...
				case Constants.TYPE_PRESSURE:
					...
					if (pressureLayout != null
							&& pressureLayout.getChildCount() > 0) {
				...
				case Constants.TYPE_STATE:
					...
					if (switchSensorLayout != null
							&& switchSensorLayout.getChildCount() > 0) {
						switchSensorLayout.addView(getSplitter());
					}
					txtview.setText(getString(((StateDevice) device)
							.getStateStringResource()));
					txtview.setTextColor(((StateDevice) device)
							.getColorByState());
					txtview.setLayoutParams(rightObjectParams);
					devicelayout.addView(txtview);
				...
				case Constants.TYPE_SWITCH:
					...
					toggle.setOnClickListener(new ToggleButtonOnClickListener(
							(SwitchDevice) device));
					if (switchControlLayout == null) {
						switchControlLayout = setLayout(switchControlLayout, ID);
					}
					if (switchControlLayout != null
							&& switchControlLayout.getChildCount() > 0) {
				...
				case Constants.TYPE_ILLUMINATION:
					...
					if (illuminationLayout != null
							&& illuminationLayout.getChildCount() > 0) {
				...
				case Constants.TYPE_EMMISION:
					...
					if (emissionLayout != null
							&& emissionLayout.getChildCount() > 0) {
	...
	protected void renameDevice(final BaseDevice device, final TextView view) {
		AlertDialog.Builder builder = new AlertDialog.Builder(
				DataOfLocationScreenActivity.this);
				...
				.setPositiveButton("Rename",
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
								...
								// server
								boolean saved = mController.saveDevice(device,
										SaveDevice.SAVE_NAME);

								String message = saved ? String.format(
										"Device was renamed to '%s'",
										device.getName())
	...
	private LinearLayout setLayout(LinearLayout devicelayout, int ID) {
		...
		Compatibility.setBackground(devicelayout,
